...
if __name__ == '__main__':
    ...
    file_list = glob.glob(args.data+"*.mat")
    files = [args.data + "data_" + str(i+1) + ".mat" for i in range(0, len(file_list))]
    image_seq = [np.array(sio.loadmat(f)['dataToBeSaved']) for f in files]
    ...
    block.segments[0].analogsignals[0].description += 'Spiking simulation output '
